...
def build_announcement_card(ann: dict) -> dict:
    ...
    doc_url = ann.get("doc_url", "https://mops.twse.com.tw/")
...
def push_announcements(announcements: list[dict]):
    ...
    if not announcements:
        ...
        # LINE carousel 最多 12 個 bubble，第一個是摘要
        cards = [build_summary_header(len(announcements), today)]
        for ann in announcements[:11]:  # 最多再放 11 筆
            cards.append(build_announcement_card(ann))
    ...
    if resp.status_code == 200:
        print(f"[LINE] 推送成功（{len(announcements)} 筆公告）")
    else:
